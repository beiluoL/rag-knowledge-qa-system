...
public class DataInitializer implements CommandLineRunner {
    ...
    private void seedKnowledgeBases() {
        ...
        seedKb("产品与电商手册", "商品、运营、客服话术类知识库", "产品/电商手册");
    ...
    private Map<String, List<String[]>> buildSampleDocs() {
        ...
        map.put("产品与电商手册", List.of(
                new String[]{"商品上架标准流程",
                        "商品上架需依次完成：1) 填写标题与卖点，标题包含核心关键词；2) 选择正确类目，类目错误会影响搜索与活动报名；3) 设置价格与划线价，注意价格区间合规；4) 维护库存数量与预警值；5) 上传主图与详情图，主图白底、清晰；6) 编辑详情页，突出规格、材质、售后。上架前需经过风控与质检审核，审核通过后才对买家可见。"},
                new String[]{"优惠券规则与叠加逻辑",
                        "优惠券常见类型有满减券（满 X 减 Y）、折扣券（打 N 折）、无门槛券。叠加规则需明确：平台券与店铺券通常可叠加，但同类型券一般互斥。优惠门槛指订单实付金额达到 X 才可抵扣。优惠券有有效期，过期作废。限购指每个用户限领/限用张数。计算顺序一般为：先店铺优惠，再平台跨店优惠，最后使用红包。"}));
